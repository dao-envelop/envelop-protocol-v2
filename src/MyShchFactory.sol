...
contract MyShchFactory is EnvelopWNFTFactory {
    ...
    constructor(address _implementation) EnvelopWNFTFactory() {
        require(_implementation != address(0), "No zero address");
        implementations[AssetType.ERC721].push(_implementation);
    }

    function mintPersonalMSW(uint64 _tgId, bytes calldata _signature) external payable returns (address wnft) {
        ...
        if (s.isTrusted) {
            ...
            s = _isTrustedSigner(_restoreDigest(_tgId, currentNonce[_tgId] + 1), _signature);
        ...
        bytes memory initCallData = abi.encodeWithSignature(
            IEnvelopV2wNFT(implementations[AssetType.ERC721][implementations[AssetType.ERC721].length - 1])
                .INITIAL_SIGN_STR(),
            InitParams(
                msg.sender,
                "MyshchWallet",
    ...
    function mintBatchMSW(uint64[] calldata _tgIds, address[] memory _receivers, bytes calldata _signature)
        external
        payable
    ...
    function createCustomERC20(
        ...
        bytes memory initCallData = abi.encodeWithSignature(
            ...
            IEnvelopV2wNFT(erc20impl).INITIAL_SIGN_STR(),
            _creator,
            name_,
            symbol_,
            _totalSupply,
    ...
    function getAddressForNonce(uint64 _tgId, uint256 _nonce) external view returns (address wnft) {
     ...
     */
    function getDigestForSign(uint64 _tgId, uint256 _nonce) external view returns (bytes32) {
        ...
        returns (address wnft)
    {
        ...
        wnft = _cloneDeterministic(
            impl, _initCallData, keccak256(abi.encode(_tgId, currentNonce[_tgId])), _valueDenominator
        );

        emit EnvelopV2Deployment(wnft, impl, IEnvelopV2wNFT(impl).ORACLE_TYPE());
    }

    function _getAddressForNonce(uint64 _tgId, uint256 _nonce) internal view returns (address) {
    ...
    function _isTrustedSigner(bytes32 _digest, bytes calldata _signature) internal view returns (Signer memory s) {
    ...
    function _restoreDigest(uint64 _tgId, uint256 _nonce) internal view returns (bytes32 dgst) {
        dgst = MessageHashUtils.toEthSignedMessageHash(keccak256(abi.encode(_tgId, _nonce, block.chainid)));
    }
}

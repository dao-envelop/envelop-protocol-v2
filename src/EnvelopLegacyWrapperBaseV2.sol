...
contract EnvelopLegacyWrapperBaseV2 is Ownable, TokenService {
    ...
    function wrap(INData calldata _inData, ET.AssetItem[] calldata _collateral, address _wrappFor)
        external
        payable
        returns (ET.AssetItem memory wnft)
    {
        return _wrap(_inData, _collateral, NFTMetaDataPacked(_wrappFor, 1, "LegacyEnvelopWNFTV2", "LEWV2", ""));
    }
    ...
    function wrapBatch(
    ...
    ) external payable returns (ET.AssetItem[] memory wnfts) {
        ...
        for (uint256 i = 0; i < _inDataS.length; i++) {
            wnfts[i] = _wrap(
                _inDataS[i],
                _collateralERC20,
                NFTMetaDataPacked(_receivers[i], _inDataS.length, "LegacyEnvelopWNFTV2", "LEWV2", "")
            );
        }
    }
    ...
    ) public payable returns (ET.AssetItem memory wnft) {
        return _wrap(_inData, _collateral, NFTMetaDataPacked(_wrappFor, 1, name_, symbol_, _baseurl));
    }
...

    {
        ...
        for (uint256 i = 0; i < _inDataS.length; i++) {
            _wrap(
                _inDataS[i],
                _collateralERC20,
                NFTMetaDataPacked(_receivers[i], _inDataS.length, name_, symbol_, _baseurl)
            );
        }
    }
    ...
    function addCollateralBatch(
        ...
        for (uint256 i = 0; i < _wNFTAddress.length; i++) {
            ...
            if (_supportsERC165InterfaceUnchecked(_wNFTAddress[i], type(IEnvelopV2wNFT).interfaceId)) {
    ...
    //                    Admin functions                              //
    /////////////////////////////////////////////////////////////////////
    function setWNFTId(ET.AssetType _assetOutType, address _wnftContract, uint256 _tokenId) external onlyOwner {
    ...
    function saltBase(ET.AssetType _wnftType) public view returns (ET.NFTItem memory nextItem) {
        ...
        returns (ET.AssetItem memory wnft)
    {
        ...
        if (
            _inData.inAsset.asset.assetType != ET.AssetType.NATIVE
                && _inData.inAsset.asset.assetType != ET.AssetType.EMPTY
        ) {
            require(
                _mustTransfered(_inData.inAsset) == _transferSafe(_inData.inAsset, msg.sender, wnftAddress),
    ...
    function _mustTransfered(ET.AssetItem calldata _assetForTransfer) internal pure returns (uint256 mustTransfered) {
    ...
    function _addCollateral(address _wNFTAddress, ET.AssetItem[] calldata _collateral) internal virtual {
        // Process Token Colleteral
        for (uint256 i = 0; i < _collateral.length; i++) {
            if (_collateral[i].asset.assetType != ET.AssetType.NATIVE) {
                require(
                    _mustTransfered(_collateral[i]) == _transferSafe(_collateral[i], msg.sender, _wNFTAddress),
                    "Suspicious asset for wrap"
                );
            }
        }
    ...
    function _addCollateralNative(address _wNFTAddress, uint256 _batchSize) internal {
        if (msg.value > 0) {

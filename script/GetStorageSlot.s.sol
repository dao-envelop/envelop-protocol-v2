...
contract GetStorageSlot is Script {
    ...
    function run() public view {
        ...
        console2.log(
            "openzeppelin.storage.Singleton721 \n %s \n",
            vm.toString(
                keccak256(abi.encode(uint256(keccak256("openzeppelin.storage.Singleton721")) - 1))
                    & ~bytes32(uint256(0xff))
            )
        );

        console2.log(
            "envelop.storage.WNFTV2Envelop721 \n %s \n",
            vm.toString(
                keccak256(abi.encode(uint256(keccak256("envelop.storage.WNFTV2Envelop721")) - 1))
                    & ~bytes32(uint256(0xff))
            )
        );
        ...
        console2.log(
            "envelop.storage.WNFTMyshchWallet \n %s \n",
            vm.toString(
                keccak256(abi.encode(uint256(keccak256("envelop.storage.WNFTMyshchWallet")) - 1))
                    & ~bytes32(uint256(0xff))
            )
        );
    }

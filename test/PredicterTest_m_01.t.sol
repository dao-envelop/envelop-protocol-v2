...
contract PredicterTest_m_01 is Test, PredictionBuilder {
    ...
    function test_resolvePrediction_onlyYesBets_Win() public {
        ...
        l.oneReward = (totalNoAmount / yesNum - l.estTotalFeeBenef / yesNum- l.estTotalFeeCreator/ yesNum) ;
        console2.log('Estimate Reward Amount: %s', l.oneReward);  
    ...
    function test_resolvePrediction_onlyNoBets_Lose() public {
        uint40 exp = uint40(block.timestamp + 1 days);
        uint96 strikeAmount = 1_000_000;
        uint96 predictedPrice = 100;
        Predicter.Prediction memory pred = _buildPrediction(address(token), exp, strikeAmount, predictedPrice);
    ...
    function test_resolvePrediction_onlyYesBets_Lose() public {
        uint40 exp = uint40(block.timestamp + 1 days);
        uint96 strikeAmount = 1_000_000;
        uint96 predictedPrice = 100;
        Predicter.Prediction memory pred = _buildPrediction(address(token), exp, strikeAmount, predictedPrice);
        ...
        l.estTotalFeeBenef = totalYesAmount * predicter.FEE_PROTOCOL_PERCENT() * predicter.SCALE()
          /predicter.PERCENT_DENOMINATOR()/ predicter.SCALE();
        console2.log('Estimate Beneficiary Fee Amount: %s', l.estTotalFeeBenef);    

        l.estTotalFeeCreator = totalYesAmount * predicter.FEE_CREATOR_PERCENT() * predicter.SCALE()
          /predicter.PERCENT_DENOMINATOR() / predicter.SCALE();
        console2.log('Estimate Creator     Fee Amount: %s', l.estTotalFeeCreator);


        l.oneReward = (totalYesAmount / yesNum - l.estTotalFeeBenef / yesNum- l.estTotalFeeCreator/ yesNum) ;
        console2.log('Estimate Reward Amount: %s', l.oneReward);  
        
        // set oracle price > predictedPrice => predictedTrue = false (no wins), but no bets don't exist
    ...
    function test_resolvePrediction_onlyNoBets_Win() public {
        uint40 exp = uint40(block.timestamp + 1 days);
        uint96 strikeAmount = 1_000_000;
        uint96 predictedPrice = 100;
        Predicter.Prediction memory pred = _buildPrediction(address(token), exp, strikeAmount, predictedPrice);

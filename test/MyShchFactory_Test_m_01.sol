...
contract MyShchFactory_Test_m_01 is Test {
    ...
    function test_custom_erc20() public {
        ...
        assertEq(token.balanceOf(address(this)), 1_000_000e18 - initDisrtrib[0].amount - initDisrtrib[1].amount);
        assertEq(token.balanceOf(address(1)), initDisrtrib[0].amount);
        assertEq(token.balanceOf(address(2)), initDisrtrib[1].amount);
        ...
        token.burn(1e18);
        assertEq(token.balanceOf(address(4)), 0);
    }
    ...
    function test_digest() public pure {
        //  as now
        bytes32 dgst =
            MessageHashUtils.toEthSignedMessageHash(keccak256(abi.encode(uint64(222), uint256(1), uint256(11155111))));
        console2.logBytes32(dgst);

        // hash
        dgst = MessageHashUtils.toEthSignedMessageHash(abi.encode(uint64(222), uint256(1), uint256(11155111)));
        console2.logBytes32(dgst);

        dgst = keccak256(abi.encode(uint64(222), uint256(1), uint256(11155111)));
        console2.logBytes32(dgst);

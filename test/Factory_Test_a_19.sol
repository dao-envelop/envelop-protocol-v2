...
// collateral info has more eth than msg.value for tx
// expect revert
contract Factory_Test_a_19 is Test {
    ...
    function test_create_legacy() public {
        ...
        (bool sent, bytes memory data) = address(1).call{value: sendEtherAmount / 2}("");
        ...
        colAssets[0] = ET.AssetItem(ET.Asset(ET.AssetType.NATIVE, address(0)),0,sendEtherAmount);

        vm.expectRevert(
            abi.encodeWithSelector(WNFTLegacy721.InsufficientCollateral.selector, colAssets[0], sendEtherAmount / 2)
        );
        wrapper.wrap{value: sendEtherAmount / 2}(
            inData,
